...
describe('useWindowSize', () => {
	...
	test('should return current window dimensions', () => {
		...
		expect(screen.getByTestId('size').textContent).toBe('Width: 1024, Height: 768');
	});
	...
	test('should update when window is resized', () => {
		...
		expect(screen.getByTestId('size').textContent).toBe('Width: 800, Height: 600');
	});
	...
	test('should call resize handler immediately on mount', () => {
		...
		expect(screen.getByTestId('size').textContent).toBe('Width: 500, Height: 300');
	});
});
