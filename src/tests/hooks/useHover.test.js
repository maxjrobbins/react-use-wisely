	...
	return (
		<div>
			...
			<div data-testid="hover-status">
				{isHovered ? 'Element is being hovered' : 'Element is not being hovered'}
			</div>
		</div>
	);
}
...
describe('useHover', () => {
	test('should initialize with isHovered as false', () => {
		...
		expect(screen.getByTestId('hover-status').textContent).toBe('Element is not being hovered');
	});
	...
	test('should set isHovered to true on mouseenter', () => {
		...
		expect(screen.getByTestId('hover-status').textContent).toBe('Element is being hovered');
	});
