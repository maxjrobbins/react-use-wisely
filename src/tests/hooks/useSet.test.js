...
function TestComponent({ initialItems = [] }) {
	const [set, { add, remove, clear, has, toggle }] = useSet(initialItems);
				...
				<button
					...
					onClick={() => {
						...
						document.getElementById('check-result').textContent =
							result ? 'Item exists' : 'Item does not exist';
					}}
				>
					Check Item
				</button>
...
describe('useSet', () => {
	...
	test('should initialize with provided initial items', () => {
		...
		expect(screen.getByTestId('items').textContent).toBe('apple, banana, orange');
	});
	...
	test('should handle complex data types', () => {
		...
		function ComplexTypesComponent() {
			const [set, { add, has }] = useSet();
			const [objectInSet, setObjectInSet] = React.useState(null);
			const [hasOriginal, setHasOriginal] = React.useState(false);
			const [hasEquivalent, setHasEquivalent] = React.useState(false);
			...
			return (
				<div>
					...
					<button
						data-testid="check-objects"
						onClick={handleCheck}
					>
						Check Objects
					</button>
					<div data-testid="has-original">
						{hasOriginal ? 'Has original object' : 'Does not have original object'}
					</div>
					<div data-testid="has-equivalent">
						{hasEquivalent ? 'Has equivalent object' : 'Does not have equivalent object'}
					</div>
					<div data-testid="set-size">
						Size: {set.size}
					</div>
				</div>
			);
		...
		expect(screen.getByTestId('has-original').textContent).toBe('Does not have original object');
		...
		expect(screen.getByTestId('has-original').textContent).toBe('Has original object');

		// But not an equivalent object with different reference
		expect(screen.getByTestId('has-equivalent').textContent).toBe('Does not have equivalent object');
	});
});
